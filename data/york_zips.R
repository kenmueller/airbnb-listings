...
# 40 zip codes are blank
myDataFrame <- read.csv('listings-out.csv', colClasses=c(zip_code='character')) %>%
  select(
    beds,
    baths,
    bedrooms,
...
plotMCMC2 = function( codaSamples , data , sName="s" , yName="y" , 
  ...
  postInfo = plotPost( mcmcMat[,"b0"] , compVal=NULL , ROPE=NULL ,
                       xlab=expression(beta[0]) , main="" , 
                       xlim=c( min(mcmcMat[,"b0"]),
                               quantile(mcmcMat[,"b0"],probs=c(0.990)) ) )
  postInfo = plotPost( mcmcMat[,"aMet"] , compVal=compVal , ROPE=rope ,
                       xlab=expression(beta[bedrooms]) , #main="Group Mode" ,
                       xlim=quantile(mcmcMat[,"aMet"],probs=c(0.005,0.995)) )
  if ( !is.null(saveName) ) {
    saveGraph( file=paste(saveName,"PostaMet",sep=""), type=saveType)
  }
  ...
  if ( !is.null(saveName) ) {
    saveGraph( file=paste(saveName,"PostB",sep=""), type=saveType)
  }
}
...
fileNameRoot = "kenparables_zips-t-"
graphFileType = "png"
...
# for (parName in parameterNames) {
for (parName in c('b[16]', 'b[75]', 'b[149]', 'ySigma[16]', 'ySigma[75]', 'ySigma[149]', 'aMet', 'b0', 'boroughMu[1]', 'boroughMu[2]', 'boroughMu[3]', 'boroughMu[4]', 'boroughMu[5]')) {
  diagMCMC(
    codaObject = mcmcCoda ,
    parName = parName ,
    saveName = fileNameRoot ,
...
#-------------------------------------------------------------------------------
# plot zip codes at positions 16, 75, and 149
# Bergen Beach: 11234
# Fresh Meadows: 11365
# SoHo: 10012
plotMCMC2(
  ...
  yName = yName ,
  sName = 'zip_code' ,
  compVal=0.0, diffIdVec=c(16,75,149), compValDiff=0.0,
  saveName = fileNameRoot ,
  saveType = graphFileType
)

plotPosterior <- function(codaSamples ,# data , 
                          compVal=0.0 , rope=NULL , 
                          diffIdVec=NULL , compValDiff=0.0 , ropeDiff=NULL , 
                          saveName=NULL , saveType="png", parName="boroughMu[1]") {
  # y = data[,yName]
  # s = as.numeric(as.factor(data[,sName])) # ensures consecutive integer levels
  # Now plot the posterior:
  mcmcMat = as.matrix(codaSamples,chains=TRUE)
  ...
  postInfo = plotPost( mcmcMat[,parName] , cex.lab = 1.75 , 
                       ...
                       xlab=parName , main="" )
  if ( !is.null(saveName) ) {
    saveGraph( file=paste(saveName,"Posterior-",parName,sep=""), type=saveType)
  }
}
for (parName in c('boroughMu[1]', 'boroughMu[2]', 'boroughMu[3]', 'boroughMu[4]', 'boroughMu[5]')) {
  plotPosterior(
    mcmcCoda ,
    compVal=0.0, compValDiff=0.0,
    saveName = fileNameRoot ,
    saveType = graphFileType,
    parName = parName
  )
